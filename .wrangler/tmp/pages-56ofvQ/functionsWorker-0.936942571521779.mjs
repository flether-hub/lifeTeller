...
async function authenticateAdmin(request, env2) {
...
var onRequestPost = /* @__PURE__ */ __name(async (context2) => {
  ...
  if (password === env2.ADMIN_PASSWORD) {
    const exp = Math.floor(Date.now() / 1e3) + 24 * 60 * 60;
    const token = await index_default.sign({ admin: true, exp }, env2.JWT_SECRET);
...
var onRequestGet = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    const pointsResult = await env2.DB.prepare(
    ...
    const provinceResult = await env2.DB.prepare(
...
// api/fortune/check.ts
var onRequestPost2 = /* @__PURE__ */ __name(async (context2) => {
  const { env: env2, request } = context2;
  const ip = request.headers.get("cf-connecting-ip") || "\u672A\u77E5IP";
  const today = getBeijingDateString();
  try {
    const totalLimit = parseInt(await getSetting(env2.DB, "total_daily_limit", "100"));
    const ipLimit = parseInt(await getSetting(env2.DB, "ip_daily_limit", "3"));
    const totalUsedResult = await env2.DB.prepare(
      "SELECT COUNT(*) as count FROM readings WHERE date(datetime(created_at, '+8 hours')) = ?"
    ).bind(today).first();
    const ipUsedResult = await env2.DB.prepare(
    ...
    const customKey = await getSetting(env2.DB, "custom_api_key", "");
...
var onRequestPost3 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const customKey = await getSetting(env2.DB, "custom_api_key", "");
    let apiKey = customKey || env2.GEMINI_API_KEY;
    if (!apiKey) {
    ...
    try {
      ...
      if (customKey && env2.GEMINI_API_KEY && (isAuthError || isQuotaError)) {
        console.warn(`Gemini API Error with custom key (${isAuthError ? "Auth" : "Quota"}), falling back to system key`);
        responseText = await tryGenerate(env2.GEMINI_API_KEY) || "";
      } else {
        throw err;
      }
    }
...
var onRequestPost4 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const { name, gender, date, time: time3, calendar_type, province, resultJson } = body;
    if (!name || !date || !time3 || !province || !resultJson) {
    ...
    ).bind(ip, name, calendar_type, date, time3, province).first();
    if (result) {
      ...
      ).bind(ip, ip_location, lat, lon, name, gender || "\u672A\u77E5", calendar_type, date, time3, province, JSON.stringify(resultJson)).run();
    }
...
var onRequest = /* @__PURE__ */ __name(async (context2) => {
  ...
  if (request.method === "GET") {
    ...
    const totalRes = await env2.DB.prepare("SELECT COUNT(*) as count FROM readings").first();
...
var onRequest2 = /* @__PURE__ */ __name(async (context2) => {
  ...
  if (request.method === "POST") {
    ...
    await env2.DB.prepare("UPDATE settings SET value = ? WHERE key = ?").bind(String(total_daily_limit), "total_daily_limit").run();
    await env2.DB.prepare("UPDATE settings SET value = ? WHERE key = ?").bind(String(ip_daily_limit), "ip_daily_limit").run();
    if (custom_api_key !== void 0) {
      await env2.DB.prepare("UPDATE settings SET value = ? WHERE key = ?").bind(String(custom_api_key), "custom_api_key").run();
    }
    return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" } });
  }
  return new Response("Method not allowed", { status: 405 });
...
// ../.wrangler/tmp/pages-56ofvQ/functionsRoutes-0.20568240616613398.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-oZXman/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-oZXman/middleware-loader.entry.ts
...
@google/generative-ai/dist/index.mjs:
   ...
   *)
*/
//# sourceMappingURL=functionsWorker-0.936942571521779.mjs.map
