...
export default function Admin() {
  ...
  const [settings, setSettings] = useState({ total_daily_limit: '', ip_daily_limit: '', custom_api_key: '' });
  ...
  if (!token) {
    return (
      <div className="min-h-screen font-sans text-slate-800 flex flex-col relative overflow-x-hidden">
        <StarryBackground />
        <Header />
        <main className="flex-1 flex items-center justify-center p-4 mt-12 z-10 w-full relative">
          <form onSubmit={handleLogin} className="bg-white/90 backdrop-blur-2xl p-8 rounded-2xl w-full max-w-sm border border-blue-100 shadow-2xl relative z-10">
            <h2 className="text-2xl text-blue-800 font-serif font-bold mb-6 text-center">系统管理后台</h2>
            <div className="mb-4">
               <input 
                 type="password" 
                 placeholder="输入管理密码"
        ...
        <div className="flex-1 overflow-auto bg-white/95 border border-blue-100 shadow-xl rounded-2xl p-4 md:p-8 scrollbar-thin scrollbar-thumb-blue-200 scrollbar-track-transparent">
           ...
           {activeTab === 'map' && (
             <div className="h-full flex flex-col">
               <h2 className="text-2xl font-serif text-blue-800 font-bold mb-6 shrink-0">用户来源地理分布</h2>
               <div className="bg-slate-50 p-6 rounded-xl border border-blue-100 shadow-sm flex-1 relative overflow-hidden flex items-center justify-center min-h-[500px]">
                  <ComposableMap projection="geoMercator" className="w-full h-full max-h-[600px]">
                    <ZoomableGroup zoom={1} center={[104, 35]} minZoom={1} maxZoom={10}>
                        ...
                        return (
                          <Marker key={i} coordinates={[loc.lon, loc.lat]}>
                            <g transform="translate(-8, -20)">
                              <path d="M12 14v4M8 18h8M12 14C8.686 14 6 16.239 6 19v1h12v-1c0-2.761-2.686-5-6-5z" fill="#4f46e5" stroke="#4f46e5" strokeWidth="1" strokeLinejoin="round" />
                              <circle cx="12" cy="8" r="4" fill="#4f46e5" />
                            </g>
                            <text
                              textAnchor="middle"
                              y={-22}
                              style={{ fill: "#0f172a", fontSize: "10px", fontWeight: "bold", pointerEvents: "none", textShadow: "1px 1px 0 #fff, -1px 1px 0 #fff, 1px -1px 0 #fff, -1px -1px 0 #fff" }}
                            >
                              {loc.ip_location}{loc.count ? ` (${loc.count})` : ''}
                            </text>
                          </Marker>
                        )
                      })}
           ...
           {activeTab === 'settings' && (
             <div className="max-w-2xl h-full flex flex-col">
               <h2 className="text-2xl font-serif text-blue-800 font-bold mb-6">系统配置</h2>
               <form onSubmit={handleSaveSettings} className="bg-white p-8 rounded-xl border border-blue-100 shadow-sm space-y-6">
                 <div>
                   <label className="block text-sm text-slate-700 mb-2 font-medium">每日全站总额度 (防刷限制)</label>
                   <input 
                     type="number" 
                     value={settings.total_daily_limit} 
                     onChange={e => setSettings({...settings, total_daily_limit: e.target.value})}
                 ...
                 <div>
                   <label className="block text-sm text-slate-700 mb-2 font-medium">单IP每日额度限制</label>
                   <input 
                     type="number" 
                     value={settings.ip_daily_limit} 
                     onChange={e => setSettings({...settings, ip_daily_limit: e.target.value})}
                     className="w-full bg-slate-50 border border-blue-200 rounded-xl p-3 text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition"
                   />
                 </div>
                 <div>
                   <label className="block text-sm text-slate-700 mb-2 font-medium">自定义 Gemini API Key</label>
                   <input 
                     type="password" 
                 ...
                 {msg && <div className="bg-green-50 text-green-700 p-3 rounded-xl border border-green-200 text-sm">{msg}</div>}
               ...
               <div className="flex items-center gap-4">
                 <button
                   onClick={async () => {
                     ...
                     try {
                       const { url } = await generateExportData('fortune-result-content', selectedReading.name, 'pdf');
                 ...
                 <button
                   onClick={async () => {
                     ...
                     setExportReadyUrl(null);
                     try {
                       const { url } = await generateExportData('fortune-result-content', selectedReading.name, 'image');
                       if (url) setExportReadyUrl(url);
                     } catch (err) {
                       console.error(err);
                       setExportModalOpen(false);
                 ...
                 <button onClick={() => setSelectedReading(null)} className="text-slate-400 hover:text-slate-600 transition ml-2">
                   <X size={24} />
                 </button>
               </div>
             </div>
                 ...
                 try {
                   ...
                   return (
                     <div className="mx-auto bg-white shadow-sm">
                       <FortuneResultView result={res} userInfo={{
      ...
      {deleteConfirmId !== null && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4 font-serif text-slate-800">确认删除？</h3>
            <p className="text-slate-500 text-center mb-8">此操作不可恢复，确定要删除这条记录吗？</p>
